...
class UDPChatClient:
    ...
    This Python client:
      ...
      => ensures command packets have source port == py_rx_port (required by some setups).
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.enable_tx_prints = bool(enable_tx_prints)
        ...
        self._log_queue: "queue.Queue[Tuple[float, Tuple[str, int], Dict[str, Any]]]" = queue.Queue(maxsize=5000)
    ...
    def _send_raw(self, msg: str) -> None:
        ...
        This is the critical fix vs. an unbound tx socket (ephemeral port).
        """
        if not self._sock:
        ...
        if self.enable_tx_prints:
            print(f"[TX] {self.py_bind_ip}:{self.py_rx_port} -> {self.rcb_rx.ip}:{self.rcb_rx.port}  bytes={sent}  msg={msg!r}")
    ...
    def start_logging_csv(
        ...
        *,
        ...
        with self._log_lock:
            ...
            self._log_thread = threading.Thread(target=self._log_loop_flat, daemon=True)
    ...
    def get_signal(self, key: str, *, use_display: bool = False) -> Tuple[Optional[float], str]:
        ...
        If use_display=True -> displayValue/displayUnit else workingValue/workingUnit.
        """
...
def demo() -> None:
    ...
    You must set:
      ...
      - RCB_RX_PORT = receive_port in the RCbenchmark JS script (56000 in your working script)
      - PY_LISTEN_PORT = send_port in RCbenchmark JS script (64126 in your script)
    """
    ...
    def on_json(obj: JSONType, addr: Tuple[str, int]) -> None:
        if isinstance(obj, dict):
            keys = list(obj.keys())
            #print(f"[JSON] from {addr}: keys={keys}")
            # print(obj)  # uncomment if you want full dict printed
    ...
    # Start logging (customizable filename/path)
    ...
    if telemetry_seen["count"] == 0:
        ...
        print("       => RCbenchmark 'send_ip' is wrong OR firewall blocks UDP 64126 on this PC.")
    else:
